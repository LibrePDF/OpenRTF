...
import org.openrtf.text.rtf.direct.RtfDirectContent;
import org.openrtf.text.rtf.document.RtfDocument;
import org.openrtf.text.rtf.parser.ctrlwords.RtfCtrlWordData;
import org.openrtf.text.rtf.parser.ctrlwords.RtfCtrlWordListener;
import org.openrtf.text.rtf.parser.ctrlwords.RtfCtrlWordMgr;
import org.openrtf.text.rtf.parser.destinations.RtfDestination;
import org.openrtf.text.rtf.parser.destinations.RtfDestinationMgr;

/**
 * The RtfParser allows the importing of RTF documents or RTF document fragments. The RTF document
 * or fragment is tokenised, font and color definitions corrected and then added to the document
 * being written.
 *
...
public class RtfParser {
    ...
    /**
     * Conversion type is an import. Uses direct content to add everything. This is what the
     * original import does.
     */
    public static final int TYPE_IMPORT_FULL = 0;

    /**
     * Conversion type is an import of a partial file/fragment. Uses direct content to add
     * everything.
     */
    public static final int TYPE_IMPORT_FRAGMENT = 1;

    /**
     * Conversion type is a conversion. This uses the document (not rtfDoc) to add all the elements
     * making it a different supported documents depending on the writer used.
     */
     ...
     *
     * <p>Contains both known and unknown. <code>ctrlWordCount</code> should equal <code>
     * ctrlWrodHandlecCount</code> + <code>ctrlWordNotHandledCount</code + <code>
     * ctrlWordSkippedCount </code>
     */
     ...
     *
     * <p>Usage: Create a parser object and call this method with the input stream and the iText
     * Document object
     *
     * @param readerIn The Reader to read the RTF file from.
     * @param doc The iText document that the RTF file is to be added to.
     * @throws IOException On I/O errors.
     * @since 2.1.3
     */
     ...
     *
     * @param readerIn The Reader to read the RTF fragment from.
     * @param rtfDoc The RTF document to add the RTF fragment to.
     * @param importMappings The RtfImportMappings defining font and color mappings for the
     *     fragment.
     * @throws IOException On I/O errors.
     * @since 2.1.3
     */
    public void importRtfFragment(InputStream readerIn, RtfDocument rtfDoc, RtfImportMappings importMappings)
            throws IOException {
        // public void importRtfFragment2(Reader readerIn, RtfDocument rtfDoc, RtfImportMappings
        // importMappings) throws IOException {
     ...
     *
     * @param listener the EventListener that has to be removed.
     * @since 2.1.3
     */
    ...
    /**
     * Casts the input reader to a PushbackReader or creates a new PushbackReader from the Reader
     * passed in. The reader is also transformed into a BufferedReader if necessary.
     *
     * @param readerIn The Reader object for the input file.
    ...
    /**
     * Imports the mappings defined in the RtfImportMappings into the RtfImportHeader of this
     * RtfParser2.
     *
    ...
    private void handleImportMappings(RtfImportMappings importMappings) {
        ...
        while (it.hasNext()) {
            ...
            this.importMgr.importColor(
                    colorNr, importMappings.getColorMappings().get(colorNr));
        }
        ...
        while (it.hasNext()) {
            ...
            this.importMgr.importStylesheetList(
                    stylesheetListNr, importMappings.getStylesheetListMappings().get(stylesheetListNr));
        }
    }
        ...
        if (this.getTokeniserState() != TOKENISER_SKIP_GROUP) {
            if (debugParser) {
                ...
                outputDebug(
                        this.rtfDoc,
                        groupLevel,
                        "DEBUG: destination=" + this.getCurrentDestination().toString());
    ...
    /**
     * Handles control word tokens. Depending on the current state a control word can lead to a
     * state change. When parsing the actual document contents, certain tabled values are remapped.
     * i.e. colors, fonts, styles, etc.
     *
    ...
    public int handleCtrlWord(RtfCtrlWordData ctrlWordData) {
        ...
        if (debugParser) {
            outputDebug(
                    this.rtfDoc,
                    groupLevel,
                    "DEBUG: handleCtrlWord=" + ctrlWordData.ctrlWord + " param=[" + ctrlWordData.param + "]");
        }
     ...
     *
     * @param nextChar The text token to handle.
     * @return errOK if ok, other if an error occurred.
     * @since 2.1.3
     */
     ...
     *
     * @return The current RtfParserState state object.
     * @since 2.1.3
     */
     ...
     *
     * @param newState The new state for the parser
     ...
     *
     * @return The type of the conversion. Import or Convert.
     * @since 2.1.3
     */
     ...
     *
     * @return Returns the object rtfDoc.
     * @since 2.1.3
     */
     ...
     *
     * @return Returns the object importHeader.
     * @since 2.1.3
     */
    ...
    public void tokenise() throws IOException {
        ...
        while ((nextChar = this.pbReader.read()) != -1) {
            ...
            {
                if ((errorCode = parseChar(nextChar)) != errOK) return;
            } else {
                //				switch(nextChar[0]) {
                switch (nextChar) {
                    ...
                    default:
                        ...
                        if (this.getTokeniserState() == TOKENISER_HEX) {
                            ...
                            hexChars.append(nextChar);
                            try {
                                //								nextChar[0]=(char)Integer.parseInt(hexChars.toString(),
                                // 16);
     ...
     *
     * @param nextChar The character to process
     * @return Returns an error code or errOK if no error.
     * @since 2.1.3
     */
    private int parseChar(int nextChar) {
         ...
         * ... document text ...
         */
        if (this.getTokeniserState() == TOKENISER_BINARY && --binByteCount <= 0) this.setTokeniserStateNormal();
        if (this.getTokeniserState() == TOKENISER_SKIP_BYTES && --binSkipByteCount <= 0) this.setTokeniserStateNormal();
     ...
     *
     * @param reader This is a pushback reader for file input.
     * @return Returns an error code or errOK if no error.
     ...
     *
     * @return The current group level value.
     * @since 2.1.3
     */
    public int getLevel() {
    ...
    /**
     * Set the tokeniser state to skip to the end of the group. Sets the state to
     * TOKENISER_SKIP_GROUP and skipGroupLevel to the current group level.
     *
     * @since 2.1.3
     */
     ...
     *
     * @param numberOfBytesToSkip The numbere of bytes to skip in the file.
     * @since 2.1.3
     */
     ...
     *
     * @param binaryCount The number of binary bytes.
     * @since 2.1.3
     */
     ...
     * @see org.openrtf.text.rtf.parser.RtfParser#TYPE_CONVERT
     * @since 2.1.3
     */
     ...
     *
     * @return true if TYPE_CONVERT, otherwise false
     * @see org.openrtf.text.rtf.parser.RtfParser#TYPE_IMPORT_FULL
     * @see org.openrtf.text.rtf.parser.RtfParser#TYPE_IMPORT_FRAGMENT
     * @since 2.1.3
     */
