...
/**
 * <code>RtfCtrlWordBase</code> is the base class for all control word handlers to extend from.
 *
...
public class RtfCtrlWordHandler implements Cloneable {
    ...
    /**
     * String containing the value of "{" or "" (blank) depending on if this is the first control
     * word in a group.
     *
     * @since 2.0.8
     */
     ...
     *
     * @see org.openrtf.text.rtf.parser.ctrlwords.RtfCtrlWordHandler#RtfCtrlWordHandler(RtfParser
     *     rtfParser, String ctrlWord, int defaultParameterValue, boolean passDefaultParameterValue,
     ...
     * Constructor:
     *
     * @param rtfParser The parser for this control word.
     * @param ctrlWord The string value of this control word.
     * @param defaultParameterValue The default value of this control word. Not all control words
     *     have values.
     * @param passDefaultParameterValue Flag indicating if this control word should use the default
     *     value.
     * @param ctrlWordType Indicator of the type of control word this is.
     *     DESTINATION|DESTINATION_EX|VALUE|FLAG|TOGGLE|SYMBOL
     * @param prefix String to prefix the ctrl word with. "\" or "\*\" are the 2 used values.
     * @param suffix String to add as suffix to the ctrl word. " " and "" are the 2 used values.
     * @param specialHandler If TOGGLE then the property name as String (propertyGroup.propertyName
     *     format ex. "character.bold") If FLAG then the property name as String
     *     (propertyGroup.propertyName format ex. "character.bold") If VALUE then the property name
     *     as String (propertyGroup.propertyName format ex. "character.bold") If SYMBOL then the
     *     character to use for substitution as String If DESTINATION|DESTINATION_EX then the
     *     RtfDestination class name as String
     * @since 2.0.8
     */
            ...
            if (this.ctrlWordType == RtfCtrlWordType.SYMBOL) {
                ...
                if (this.specialHandler == null) {
                    ...
                    if (this.specialHandler.length() > 1 && this.specialHandler.endsWith(".")) {
                        this.specialHandler += this.ctrlWord; // if string length>1 and ends with a period,
                        // it's a group. Add
                        // ctrlWord
                    }
                }
    ...
    /**
     * The primary control word handler method. Called by the parser once it has a control word and
     * parameter if applicable.
     *
     * @param ctrlWordDataIn The control word and associated parameter if applicable.
     * @return <code>true</code> or <code>false</code> if the control word was handled.
     * @since 2.0.8
     */
    public final boolean handleControlword(RtfCtrlWordData ctrlWordDataIn) {
        ...
        if (debug) {
            ...
            RtfParser.outputDebug(
                    this.rtfParser.getRtfDocument(),
                    this.rtfParser.getLevel() + 1,
                    "RtfCtrlWordHandler debug Start: " + this.ctrlWordData.ctrlWord + " ");
        }
        ...
        switch (this.ctrlWordType) {
            ...
            case RtfCtrlWordType.SYMBOL:
                ...
                if (dest != null) {
                    String data = null;
                    // if doing an import, then put the control word in the output stream through
                    // the
                    // character handler
                    ...
                    // If no substitute character, then provide special handling in the destination
                    // for the
                    // ctrl word.
                    if (data != null) {
        ...
        if (debug) {
            RtfParser.outputDebug(
                    this.rtfParser.getRtfDocument(),
                    this.rtfParser.getLevel() + 1,
                    "RtfCtrlWordHandler debug End: " + this.ctrlWordData.ctrlWord + " ");
        }
     ...
     *
     * <p>If return value is true, no further processing will be performed on this control word.
     *
     ...
     *
     * @return <code>true</code> if control word was handled, <code>false</code> if it was not
     *     handled.
     * @since 2.0.8
     */
