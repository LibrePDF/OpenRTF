...
public class RtfListLevel extends RtfElement implements RtfExtendedElement {
    ...
    /**
     * Constant which specifies (1) if list numbers from previous levels should be converted to
     * Arabic numbers; (0) if they should be left with the format specified by their own level's
     * definition.
     */
    private static final byte[] LIST_LEVEL_LEGAL = DocWriter.getISOBytes("\\levellegal");

    /**
     * Constant which specifies (1) if this level does/does not restart its count each time a super
     * ordinate level is incremented (0) if this level does not restart its count each time a super
     * ordinate level is incremented.
     */
    ...
    public static final int LIST_TYPE_ARABIC_LEADING_ZERO =
            1022; /* 22	Arabic with leading zero (01, 02, 03, ..., 10, 11)*/
    ...
    */
    ...
    /**
     * Flag to indicate if the levellegal control word should be emitted. true if any list numbers
     * from previous levels should be converted to Arabic numbers; false if they should be left with
     * the format specified by their own level definition.
     */
    ...
    public void writeDefinition(OutputStream result) throws IOException {
        ...
        //        document.getDocumentHeader().getColorNumber(new
        // RtfColor(this.document,this.getFontNumber().getColor()));
        result.write(intToByteArray(document.getDocumentHeader()
                .getColorNumber(new RtfColor(this.document, this.getFontNumber().getColor()))));
